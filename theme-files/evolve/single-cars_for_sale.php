...
wp_enqueue_script('evolve-style-js', WP_CONTENT_URL . '/plugins/car-demon-styles/theme-files/evolve/evolve_js.php');
wp_enqueue_style('evolve-style-single-car-css', WP_CONTENT_URL . '/plugins/car-demon-styles/theme-files/evolve/evolve-style-single-car.css');
	...
	foreach($thumbnails as $thumbnail) {
		...
		if($pos == true) {
			if ($cnt < $stop) {
				...
				$photo_array = '<img class="evolve_thumbs" style="cursor:pointer" onClick=\'MM_swapImage("'.$car_title.'_pic","","'.trim($thumbnail).'",1);active_img('.$cnt.');\' src="'.trim($thumbnail).'" width="53" />';
				$this_car .= $photo_array;
				$cnt = $cnt + 1;
			}
		}
...
<div class="evolve_car_body" id="tab1">
	<div class="photos">
