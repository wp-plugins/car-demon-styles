...
include('boulevard-style-single-car.php');
include('boulevard_js.php');
	...
	foreach($thumbnails as $thumbnail) {
		...
		if($pos == true) {
			if ($cnt < $stop) {
				...
				$photo_array = '<img class="boulevard_thumbs" style="cursor:pointer" onClick=\'MM_swapImage("'.$car_title.'_pic","","'.trim($thumbnail).'",1);active_img('.$cnt.');\' src="'.trim($thumbnail).'" width="53" />';
				$this_car .= $photo_array;
				$cnt = $cnt + 1;
			}
		}
...
<div class="boulevard_car_body" id="tab1">
	...
	<div class="photos">
		...
		<div id="photo_thumbs" class="photo_thumbs">
		<?php echo $this_car; ?>
		</div>
	</div>
