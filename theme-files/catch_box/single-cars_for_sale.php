...
include('catch_box-style-single-car.php');
include('catch_box_js.php');
	...
	foreach($thumbnails as $thumbnail) {
		...
		if($pos == true) {
			if ($cnt < $stop) {
				...
				$photo_array = '<img class="catch_box_thumbs" style="cursor:pointer" onClick=\'MM_swapImage("'.$car_title.'_pic","","'.trim($thumbnail).'",1);active_img('.$cnt.');\' src="'.trim($thumbnail).'" width="53" />';
				$this_car .= $photo_array;
				$cnt = $cnt + 1;
			}
		}
...
<div class="catch_box_car_body" id="tab1">
	<div class="photos">
    ...
    <div class="car_description">
		...
		<div class="price">
			<?php echo get_vehicle_price_style_catch_box($post_id); ?>
		</div>
	</div>
	...
	<div class="tab_container" style="margin-top:10px;">
		<div id="nav" class="tab fix">
			<ul id="tablist">
				<li onclick="switch_tab('1');" id="tab_title_1" class="tab_inactive tab_active"><a>Description</a></li>
				<?php 
				$total_tabs = 1;
				if ($vin_query == 1) { 
					$total_tabs = 6;
