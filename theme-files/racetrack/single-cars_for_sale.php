...
include('racetrack-style-single-car.php');
include('racetrack_js.php');
	...
	foreach($thumbnails as $thumbnail) {
		...
		if($pos == true) {
			if ($cnt < $stop) {
				...
				$photo_array = '<img class="racetrack_thumbs" style="cursor:pointer" onClick=\'MM_swapImage("'.$car_title.'_pic","","'.trim($thumbnail).'",1);active_img('.$cnt.');\' src="'.trim($thumbnail).'" width="53" />';
				$this_car .= $photo_array;
				$cnt = $cnt + 1;
			}
		}
...
<div class="racetrack_car_body" id="tab1">
	<div class="car_contact">
		...
		<img style="position:absolute;margin-top:70px;margin-left:-70px;" src="<?php echo $car_demon_pluginpath; ?>images/race_flag.png" />
	</div>
	<div class="photos">
		<div class="main_photo">
			<img title="<?php echo $car_head_title; ?>" onclick="open_car_demon_lightbox('<?php echo $car_title; ?>_pic');" onerror="ImgError(this, 'no_photo.gif');" id="<?php echo $car_title; ?>_pic" name="<?php echo $car_title; ?>_pic" class="car_demon_main_photo" width="358px" height="271" src="<?php echo wp_get_attachment_thumb_url( get_post_thumbnail_id( $post_id ) ); ?>">
		</div>
    ...
    <div class="car_description">
		...
		<div class="price">
			<?php echo get_vehicle_price_style_racetrack($post_id); ?>
		</div>
	</div>
	<div class="tab_container">
		<ul id="tablist">
			<li onclick="switch_tab('1');" id="tab_title_1" class="tab_inactive">Description</li>
			<?php if ($vin_query == 1) { ?>
				<li onclick="switch_tab('2');" id="tab_title_2" class="tab_inactive">Specs</li>
				<li onclick="switch_tab('3');" id="tab_title_3" class="tab_inactive">Safety</li>
				<li onclick="switch_tab('4');" id="tab_title_4" class="tab_inactive">Convenience</li>
				<li onclick="switch_tab('5');" id="tab_title_5" class="tab_inactive">Comfort</li>
				<li onclick="switch_tab('6');" id="tab_title_6" class="tab_inactive">Entertainment</li>
			<?php } ?>
			<?php if ($about == 1) { ?>
				<li onclick="switch_tab('7');" id="tab_title_7" class="tab_inactive">About</li>
			<?php } ?>
		</ul>
	</div>
	<div class="tab_content_area_container">
		...
		<?php } ?>
		<?php if ($about == 1) { ?>
				<div class="tab_content tab_inactive_content" id="tab_<?php echo $about_cnt;?>">
	...
	function switch_tab(num) {
		...
		while (i<total_tabs) {
			if (i != num) {
				...
				document.getElementById("tab_title_"+i).style.borderBottom = "none";
			}
			else {
				document.getElementById("tab_"+i).style.display = "inline";
				document.getElementById("tab_title_"+i).style.background = "white";
				document.getElementById("tab_title_"+i).style.borderBottom = "solid";
				document.getElementById("tab_title_"+i).style.borderBottomColor = "white";
			}
			i++;
		}
	}
