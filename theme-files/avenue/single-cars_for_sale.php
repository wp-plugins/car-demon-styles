...
wp_enqueue_script('avenue-style-js', WP_CONTENT_URL . '/plugins/car-demon-styles/theme-files/avenue/avenue_js.php');
wp_enqueue_style('avenue-style-single-car-css', WP_CONTENT_URL . '/plugins/car-demon-styles/theme-files/avenue/avenue-style-single-car.css');
get_header();
	...
	foreach($thumbnails as $thumbnail) {
		...
		if($pos == true) {
			if ($cnt < $stop) {
				...
				$photo_array = '<img class="avenue_thumbs" style="cursor:pointer" onClick=\'MM_swapImage("'.$car_title.'_pic","","'.trim($thumbnail).'",1);active_img('.$cnt.');\' src="'.trim($thumbnail).'" width="53" />';
				$this_car .= $photo_array;
				$cnt = $cnt + 1;
			}
		}
...
<div class="avenue_car_body" id="tab1">
	<div class="photos">
